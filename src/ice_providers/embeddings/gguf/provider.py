...
class GGUFEmbeddingModel(EmbeddingModel):
    """
    Embedding provider per modelli GGUF esposti via API OpenAI-compat (/v1/embeddings).

    Tipicamente usato con:
    - llama.cpp server
    - vLLM
    - server custom OpenAI-like
    ...
    def __init__(
        ...
        *,
        ...
        self._base_url = (
            base_url
            or os.getenv(
                "ICE_EMBEDDINGS_GGUF_URL",
                "http://127.0.0.1:8000/v1",
            )
        ).rstrip("/")

        self._default_model = default_model or os.getenv(
            "ICE_EMBEDDINGS_GGUF_MODEL",
            "local-embedding-model",
        )

        self._api_key = api_key or os.getenv("ICE_EMBEDDINGS_GGUF_API_KEY")
    ...
    def encode(
        ...
        **kwargs: Any,
        ...
        # Robustezza: non assumiamo allineamento perfetto
